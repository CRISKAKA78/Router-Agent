...
## 当前改造：成员配置与自动核实（2026-09-12）

当前新建行为以 ADR-066 和 [改造验证](OVERLAY_REDESIGN_VERIFICATION.md) 为准。下文 ADR-064 的默认 multi_thread、网络级 routes、添加单设备与独立链路页描述是首轮历史；旧网络保留其默认值与身份，不能把新默认静默覆盖已有实例。

完整新流程：新建网络/密码/虚拟网段 → 选择在线未组网设备 → 首批指定固定地址成员 → 持久化稳定 machine/instance 映射及独立操作 → Probe 检查安装/必要时兼容包投放/启动 → 等待配置服务接入 → 写入/启用唯一实例 → 核实运行、实际虚拟地址和配置 → 标记完成。其他 DHCP 成员等待固定成员确认后加入。成员配置保存触发该实例的停止确认、重新应用、运行与配置确认；已停止成员不启动。

周期性只读观察与核实替代正常流程中的反复手点回查。确有未知结果时不重放写入；操作详情保留原因、Task IDs 与被停止替代关系。停止确认后可移除目标成员，其他成员未知操作不再阻塞。已成功配置、期望运行且管理在线的成员若丢失临时引擎，后台退避恢复；健康实例不重复下发，停止成员不恢复。

普通成员页不展示网络密码。通过“查看网络密码”按需查询/显式复制。关闭平台成员记录不能撤销设备已经获知的共享密钥；实际部署前仍需按原安全方案审查访问范围。
