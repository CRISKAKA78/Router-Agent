...
新配置默认http://47.119.168.150:8888，保留已有保存地址。已移除SSH账号/密码与DPAPI代码，旧ssh_user只在加载时剔除，不读取旧密码文件。维护关闭、到期或会话失效后隐藏地址；文件默认/tmp/root。560项检查与67份实际布局及产物见[本轮验证](SERVER_DEFAULTS_VERIFICATION.md)，下文旧默认/tmp及SSH凭据为历史实现。
