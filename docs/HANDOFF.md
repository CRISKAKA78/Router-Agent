...
Phase 0～5已验收。Phase 6 Windows UI从main稳定基线 `57c2b1f8f6da1069e4a2eb988224b94bafe9cf84` 开始，采用Accepted ADR-024；实现及规定验证通过，独立commit推送main后等待用户验收。当前事实见[PROJECT_STATUS](PROJECT_STATUS.md)和[PHASE6_VERIFICATION](PHASE6_VERIFICATION.md)，不得自行进入Web/微信/MCP/AI。
...
- Windows使用、部署与构建：[windows/README](../windows/README.md)和[windows/build.ps1](../windows/build.ps1)。设计：[PHASE6_DESIGN](PHASE6_DESIGN.md)、ADR-024。最终验证：[PHASE6_VERIFICATION](PHASE6_VERIFICATION.md)。
- `windows/RouterWorkbench`：原生Windows Forms工作台；`RouterWorkbench.Core`：公开HTTP DTO、ApiClient、WorkspaceConnection、配置与外部入口启动。无Server内部引用；测试专用Protocol v1对端只在Tests项目。
...
Windows设置保存到 `%LOCALAPPDATA%\RouterWorkbench\profile.json`，仅地址/客户端路径/用户名；未来认证设计从该配置及集中网络层扩展。外部窗口由用户拥有；工作台退出不杀这些程序、不主动撤销Server已有任务或维护。无密码或Tunnel内部token持久化。
...
- 本轮起点/Phase 5稳定提交：`57c2b1f8f6da1069e4a2eb988224b94bafe9cf84`。
