...
2026-09-11 用户已批准先执行A并优先GOST v3；已完成本机有限PoC，仍未通过完整准入。先读[GOST_V3_POC](GOST_V3_POC.md)和[复现入口](../tests/poc/gostv3/README.md)：固定v3.3.0/x0.16.0，TCP串口可配置接入，UDP/资源/生命周期有实测缺口。用户随后指定192.168.5.222及/tmp/root适配包继续A；本次直连SSH在认证前超时，独立TCP22检查失败（本机10.1.1.22经10.1.1.254），尚未读到压缩包或验证ARM/物理UART。恢复该路径后继续设备PoC；保持ADR-059 Proposed，不自动进入B或切换自研。
