...
日期：2026-09-06。本记录为ADR-022修正后的当前验收。修正起点 `f92d73a0d6003835993967848f1f5fe009a0df89`（Phase 4首版）；git fetch后HEAD/main/origin/main一致、工作区干净。首版起点为Phase 3 `ceaaab791850746911f965167c885789444efd4c`；首版验证历史可通过首版Git提交查看。FRP放弃方向仅保留DECISIONS.md ADR-020归档摘要，不作为本轮验收依据。

**本轮Phase 4修正及规定验收通过。**设计为Accepted ADR-021 / ADR-022，当前设计见PHASE4_DESIGN，正式wire见PROTOCOL，内部用例/配置见API。独立修正提交标题 `fix: isolate Phase 4 ports and decouple maintenance revocation`，真实SHA与推送结果由Git记录提供；推送main后停止等待验收，不进入Phase 5。
...
| 三服务同时、多个Web连接、多设备 | TestTunnelRealProbeLifecycleConcurrencyAndControlLoad：两真实Probe，A的6条流分布三服务，B已有流保持；实际协议测试显式配置双方16条并发，同时执行8个HTTP客户端和SSH/Telnet |
...
| 控制writer阻塞时pending总期限 | TestPendingDeadlineRevokesSocketWhileControlWriterBlocked：命令已准入但模拟未送达，40ms期限仍实际关闭外部socket |
...
| listener释放/端口隔离复用/部分分配回滚 | TestPortQuarantineAndDelayedReuse：三端口池关闭后立即Create失败、旧客户端重连失败、隔离到期可复用、历史淘汰不移除隔离；真实连续5轮使用不同端口；不足三端口整个分配回滚 |
...
| Close独立于控制writer/队列满 | TestMaintenanceReleaseIndependentOfBlockedControlAndFullQueue：占住writer准入、填满64项队列，含active/pending的Close在500ms内完成；数据端收到reset，Connections归零；释放writer后取消的CONNECT不写出；替换测试等待旧Gateway worker退出 |
| 不送达CLOSE、half-close后reset | TestTunnelRealDefaultCapacityAndResetAfterHalfClose：丢弃全部控制CLOSE、保持8个本地应用空闲，含已读EOF流；关闭后Probe fd/thread回到基线，既有control exec仍成功 |
| 域名/默认值/非法配置 | TestDataHostResolutionAndDefaults：localhost解析并派发IP、IP/域名格式及取消解析、24h idle/隔离和8条配额；真实HTTP/SSH/Telnet以DataHost=localhost运行 |
| 轻量默认容量与显式提高 | C++tunnel_tests校验默认8、拒绝0/65；真实Probe允许8条同时流，第9条busy关闭，线程增加不超过8；真实协议测试显式配置16以容纳更多浏览器并发 |
| 整连接idle、单向流与缓冲EOF | TestWholeConnectionIdleAndOneWayAfterHalfClose：单向进展超过短idle仍可传，停止后关闭；TestTunnelRealOneWayIdleAndBufferedEOF：真实Probe half-close后单向持续1.2s超过500ms idle，随后慢读512KiB并验证普通EOF无字节丢失 |
...
| Linux全量Go、真实Release Probe，Phase 1～4 | 全部通过，集成169.774秒 |
| ASan+UBSan+LSan CTest，detect_leaks=1 | 通过，4/4，5.97秒，无报告 |
| ASan真实Probe Phase 4全部用例 | 通过，14.905秒，无ASan/UBSan报告 |
| TSan CTest，halt_on_error=1 | 通过，4/4，6.85秒，无报告 |
| 全量Go race + TSan真实Probe，Phase 1～4 | 全包通过，集成177.103秒，无race/TSan报告 |
| Windows原生所有源码包测试、vet、Server构建 | 通过；Linux Probe用例在Windows跳过，在上述Linux实际执行 |
| Linux vet、Server构建 | 通过 |
| Close独立释放、端口隔离、域名与默认值 | Windows普通与Linux race通过，已包含于全量回归 |
| git diff --check | 通过 |

真实Probe由测试清理终止；LSan的正常进程退出检查由CTest覆盖。数据worker的实际释放另以/proc fd/thread回落及本地/远端socket终止验证，不把进程Kill本身当成无泄漏证据。

短隔离/idle用例使用显式毫秒级测试配置；产品24小时默认值由配置断言验证，不声称实际等待24小时。端口隔离验证不等价于永久旧客户端身份隔离，超窗和重启边界按ADR-022。
