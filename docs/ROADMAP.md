...
- [x] 2026-09-11 ADR-060确认Windows无额外依赖的TCP首包注册；实现独立鉴权/独占/撤销模块与PoC入口，Windows重复回归和Linux race通过。
...
- [x] 2026-09-11 创建 `codex/lan-serial-tunnel-plan`，完成[内网穿透/串口透传方案调研与计划](LAN_SERIAL_TUNNEL_PLAN.md)，记录Proposed ADR-059；无业务实现、部署或Git提交/推送。
...
- [-] 历史A完整准入未全部通过，用户已放行继续产品接入：客户端首包方案、climiter及PTY重开已有新证据；剩余UDP压力/硬件/长期监管见顶部（RSS门槛已暂停），不用历史A-only语句覆盖ADR-060授权。
