...
- [ ] 后续产品Service/API/Probe/WPF与完整生命周期接入尚未实现；按ADR-060新授权及依赖推进，不重复索取已确认的客户端选择。
...
- [ ] A完整准入仍未通过：客户端首包方案、climiter及PTY重开已有新证据；剩余UDP压力/资源/硬件/长期监管见顶部，不用历史A-only语句覆盖ADR-060授权。
