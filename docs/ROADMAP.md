...
- [ ] A完整准入：真实ARM/UART、UDP透明性与串口独占、资源预算、会话/进程撤销及长期验证尚未通过；用户已指定192.168.5.222及/tmp/root适配包，续测的SSH在认证前超时、TCP22不可达，包与设备尚未检查；恢复访问后继续A，不进入B～D。此项不是新阶段或Phase6最终验收，其他暂缓能力保持。
