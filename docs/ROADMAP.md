...
- [x] 分别保存组网、FM160、GOST未提交源码；其他现有分支提交已在main，无需重复合并。
- [x] 合并共享Server/Gateway/Probe/WPF与文档冲突，统一ADR编号，保留原默认值和业务入口。
- [x] Windows Go、WPF669项、生成器198项、Linux CTest18项、Go race及真实Probe/GOST/PTY/EasyTier定向验证；补充双WS主题与就绪后资源收敛回归。
- [x] Linux最终完整回归/vet/build通过，全部来源与冲突修正纳入main合并提交；[集成记录](ALL_WORKTREES_INTEGRATION.md)已同步。
- GitHub main推送使用本次明确授权，最终提交号及远端一致性以Git核对为准；不推送无关分支或产物。
- [-] C++ ASan/UBSan因WSL缺库受阻；不因此安装库、改变设备或替代实机验收。
- [-] UDP既有问题、物理UART/不同网关/长期稳定性及双设备组网现场验收保留；不增加新阶段或生产部署。

## 来源工作树里程碑（历史证据，非本轮最终验证）

- [x] 2026-09-12 按新授权完成GOST Service/API、Probe监管、WPF两页及Windows/Linux/ARM构建；WPF574项、15项CTest、7包race、真实Probe/GOST/PTY及实际1分钟租期通过。[产品交付](FORWARDING_IMPLEMENTATION.md)。
- [ ] 新产品包生产安装、物理UART/不同默认网关终端及1小时长稳验收；本轮不替换生产进程。
- [-] UDP丢包/延迟根因及RSS预算用户明确暂停，不阻塞产品接入，不标记修复。下方旧“尚未接入/完整准入”以本条和专项当前结果为准。

- [x] 2026-09-12 用户最新SSH改回20001，已登录并续测；RSS不再阻塞。新增有界逐包/socket计数及迟到观察，保留3秒失败判定；实机40/40最终原样返回，13个迟到≤4.821秒，目标drops=0，清理通过；[证据§4.6](GOST_V3_POC.md#46-udp突发定位与ssh20001恢复2026-09-12)。Windows157/160未宣称全通过。以下为历史里程碑；分支ADR-059/060对应main统一062/063。
...
- [-] 历史PoC剩余准入：UDP根因暂停；产品监管已实现，真实UART/不同网关/长稳转顶部实机项。RSS候选预算已由2026-09-12用户明确暂不限制；不将PTY通过当作硬件验收，不削减LAN UDP。
...
- [x] 产品Service/API/Probe/WPF及生命周期已于2026-09-12按新授权接入；实机验收边界见顶部。

- [x] 2026-09-11 创建 `codex/lan-serial-tunnel-plan`，完成[内网穿透/串口透传方案调研与计划](LAN_SERIAL_TUNNEL_PLAN.md)，记录Proposed ADR-062（来源分支原059）；无业务实现、部署或Git提交/推送。
...
- [-] 历史A完整准入未全部通过，用户已放行继续产品接入：客户端首包方案、climiter及PTY重开已有新证据；剩余UDP压力/硬件/长期监管见顶部（RSS门槛已暂停），不用历史A-only语句覆盖ADR-063/069授权。

## 当前里程碑：组网交互与生命周期改造（ADR-066）

- [x] 最新方案已授权；新建网络默认/密码、固定地址成员与多选加入、成员独立参数及修订。
- [x] 稳定设备映射、临时引擎恢复、成员隔离停止/移除、正常与异常统一配置核实，保留未知操作历史。
- [x] WPF左侧观察设备、11列成员信息、中文NAT、拓扑与流量动画；官方2.6.4真实隔离验证及Go/WPF回归，见[验证](OVERLAY_REDESIGN_VERIFICATION.md)。
- [ ] 按后续明确授权部署新Server/WPF，核实现场原未知记录及实际成员移除；真实双设备/掉电恢复/直连中继/路由/MTU验收。
- [-] 不随本轮进入二层、TLS/RBAC或正式公网部署；不自行提交/推送或改动现网。
## 当前工作树里程碑：AT模块自动适配

- [x] 独立工作树、FM160手册/ETSI核对、基础telemetry规则、精简蜂窝页与三图（ADR-067）。
- [x] 按追加授权完成details能力/24项查询、详细小区/邻区/PDP/CA/温度、LTE SINR及锁频段/频点/小区只读展示（ADR-068）；旧默认与版本兼容。
- [x] 20004只读实机、Linux全量/CTest、重点race、生成器198、WPF243和双旧工具链/Windows构建；[证据及限制](FM160_DETAILS_VERIFICATION.md)。
- [ ] 用户真实页面体验、新Server/Probe部署及生产模板持续采样验收；目前没有部署授权。
- [ ] 在可用环境补验完整WPF剪贴板及sanitizer；不把专项通过当全量通过。
- [ ] 按独立手册/样本适配FM150/FM650/RM500U/RM520N/RM500Q。
- [-] 锁定写操作、主动扫网、持久化历史、热更新规则、强制选口不纳入此次采集展示。
...
- [x] 合并后Windows Go/vet、WPF617项/81份布局及发布、Linux18项CTest/完整真实Probe集成/五包race/定向AT与组网race/vet通过；已提交到本地main，未推送，见[集成审核](OVERLAY_INTEGRATION.md)。
- [ ] 上游配置服务/账号与仓库兼容ELF包、厂商Probe构建及两台真实设备三层互通/恢复/MTU验收。
- [ ] 三层实机通过后实施二层；不默认桥接LAN，不将合并视作生产上线或Phase6最终验收。
...
- [x] 2026-09-11 异地组网方案用户审核通过，按ADR-064开始实现；不改变Phase 6验收或启动Phase 7/8。
