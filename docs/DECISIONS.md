...
> 2026-09-12 全工作树集成编号映射：main 的 ADR-066（组网改造）保留；AT 工作树原 ADR-066（FM160 遥测）改为 ADR-067，原 ADR-067（FM160 详情）改为 ADR-068。仅消除编号碰撞，不改变已接受的内容或授权范围；来源提交保留原文。用户本次已授权所有工作树源码提交、合入 main 并推送 GitHub，取代本轮来源文档中的“不合入/不提交/不推送”历史停止条件，不授权生产部署。

## ADR-069 GOST 穿透产品接入（Accepted，2026-09-12）

本决定局部取代 ADR-062/063 中“仅 PoC / 产品接入待准入”的范围限制，其余已接受的注册和会话约束保留。

用户明确UDP丢包“不管网络还是程序先不管，接着推进，完成功能和UI”，此前已暂停RSS限制。授权从PoC进入Server/API、Probe监管与WPF，取代历史A-only/全部压力准入后才接入的停止条件；不把旧UDP失败标记解决。分支旧ADR-059/060对应main062/063，不重排其他工作树编号。

接受独立Forwarding服务、0x50/0x51控制、forwarding_v1可选能力、Linux Go侧车/GOST数据面；旧Maintenance/RMT1不变。LAN直连IPv4 TCP/UDP绑定源IP代理，不改系统SNAT；串口TCP注册到loopback私有后端，当前8数据位/1停止位、波特率/校验可选。默认240分钟、0不限时但Session撤销，不恢复旧映射，端口关闭24小时持久隔离。Relay固定证书TLS只保护数据段，不代表全局API/RBAC/控制TLS改造。明文外部注册风险沿用用户所选方案。

本轮本地实现/构建/隔离测试及授权编译机产物下载，未替换生产、未提交/合main。硬件UART/不同网关/长稳仍待验收，详见[产品接入](FORWARDING_IMPLEMENTATION.md)。
...
## ADR-068 FM160 详细采集与锁定配置展示

- 状态：Accepted，2026-09-12。依据用户明确要求“把这些详细信息都适配到FM160”及“加入锁频段、锁频点、锁小区，没锁显示否，锁了显示实际锁的值”。仅扩展采集/展示，不授权执行锁定或解锁。
- Supersedes：局部取代ADR-067对完整服务小区/CA和LTE SINR的本轮范围限制。旧身份v1与telemetry v2兼容、默认关闭、独立工作树、不部署及其他未决边界保持；历史ADR不改写。
- 新增可选`cellular_probe.details=true`（必须同时telemetry=true），能力`cellular_details_v1`、独立`cellular_details` EVENT与`fibocom-fm160-details-v1`规则；24条固定查询。不向旧Probe下发新规则，不接收任意AT脚本。精确列表、载荷/时间上限见PROTOCOL。
- 详情包含温度/电压、附着、PDP/APN/IPv4/IPv6/DNS/网关、服务小区/邻区、PCC/SCC/MIMO/调制、已开启的射频测量，以及现有锁定配置。Server统一解析，API字段增加可选name/group/source；WPF消费字段，不复制AT逻辑。
- 锁频段以GTACT当前允许频段与该设备返回的支持集合对照，按已报告制式家族判断限制，不能把当前驻留B/n频段当锁定。GTCELLLOCK mode=0时锁频点/锁小区均为“否”；type=1显示制式与ARFCN且锁小区为“否”；type=0显示ARFCN与PCI，并显示可返回的SCS/NR频段。读取失败、未知模式或格式不可判定时明确缺测，不伪造“否”。
- 温度使用手册定义的MTSM=1/6/7单次测量。先读MTSM?，只在返回0/1/6/7且串口独占条件成立时执行；报告2/3或无法判断时跳过，不改周期上报，不发送停止/恢复配置。不是把MTSM?报告配置值当温度。
- 不调用GTACT/GTCELLLOCK写入、GTCELLINFO=1、GTSCANSTAT/GTCELLSCAN、拨号/APN设置或重启。GTCCINFO手册要求先GTSCANSTAT但没有提供其定义，本轮只读取模块现有报告，不保证其中邻区测量的新鲜度。GTCELLINFO关闭时不擅自开启。
- V1.3缺项/矛盾和实机变体显式记录：NR rat=9必须有NR或EN-DC标题支持；GTCCINFO按手册十六进制解析相应字段，LTE邻区兼容手册示例空保留槽；CA中不明确的数值进制/RSRP映射保留原值，未知带宽编码不猜测MHz。Power单位手册未定义，返回时展示原值，不冒称dBm。
- 查询结果、配置状态、当前网络状态分开；没有SCC报告不等于设备永远不支持CA。实现、手册页与实机/夹具覆盖区别见[详细验证](FM160_DETAILS_VERIFICATION.md)。

## ADR-067 FM160 扩展只读采集与蜂窝趋势页

- 状态：Accepted，2026-09-12。用户提供FM160/FG160 AT手册V1.3，并要求蜂窝页自动端口简化、刷新保留滚动、精简字段和RSRP/SINR/RSRQ曲线。本轮在独立工作树落实FM160-CN及必要调用链，不自动合入、推送或部署。
- Supersedes：局部取代ADR-060“只采身份”和蜂窝页诊断行/恒显端口选择器的限制；保留旧v1配置与EVENT、默认关闭、只读固定指令、独占/占用复核、会话/版本隔离、公开API及控制链路边界。
- 同一模板描述采集内容而非路由器/模组型号。新增显式`cellular_probe.telemetry=true`；默认false并省略。新Probe声明`cellular_telemetry_v2`，Server/生成器/WPF应用模板按能力门禁；旧模板仍走v1，不能静默给旧Probe开启新采集。
- Probe逐USB模块、逐轮通用身份识别后匹配内置规则；首个规则仅匹配Fibocom Wireless Inc.的完整`FM160-CN`型号。固件随身份保留，当前实测固件见专项记录；不是声称其他固件全部验证。未知型号只查询通用身份，不遍历所有厂商指令。
- 固定8项CPIN/CCID/CIMI/COPS/CEREG/C5GREG/CSQ/CESQ查询；Probe保存有界原始响应，Gateway验证版本、配置和白名单，Device Service纯函数完成归一化及单位换算，客户端不重复解析AT。超时停止当轮后续查询；ERROR不等同永久不支持。没有任意AT、网络设置、主动扫网、拨号、重启或解锁SIM能力。
- NR CESQ字段顺序及SS-RSRP映射由ETSI TS127007 V16.9.0 §8.69补齐V1.3手册缺项；完整来源、公式、255未知值和饱和边界见[FM160验证](FM160_TELEMETRY_VERIFICATION.md)。值为量化区间时用中点作曲线并显示`≈`，不是伪造精确测量；上下饱和保留`<`/`≥`。
- 自动选中端口时不显示选择器；没有自动选中时只能选择已发现候选的**观测结果**，不能因此强占端口或下发任意路径。属性表只保留当前路径及已查询内容，必要错误/过期状态放简短状态栏。原始响应仍可通过公开API取得，不在属性表堆叠命令状态。
- 三图顺序RSRP/SINR/RSRQ，使用制式固定范围，不按历史样本动态缩放。每指标优先NR，其次LTE；标题标明制式，不拼成虚构NSA信息。无SINR来源的LTE不伪造曲线。最多180点，缺失断线、重复HTTP快照不追加；设备/会话/版本/USB模块/IMEI或制式单位变更清空。多个模块的字段同时列出，当前三图只取首个模块，不混合曲线。
- 普通快照原位更新行对象、保留滚动与选中；结构变化才重绑并恢复锚点。共享检查器避免重复设置空Filter导致刷新重置；原有完整值面板和搜索保持。
- 本轮不实现FM150/FM650/RM500U/RM520N/RM500Q规则、模块规则热更新、历史数据库、强制手选采集口或完整服务小区/CA参数。
