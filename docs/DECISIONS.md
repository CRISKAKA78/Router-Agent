...
- 状态：**Proposed（2026-09-11，待用户审核，不是Accepted）**。
...
- 产品待审：所选Probe直连IPv4 IP:端口的全代理，不修改iptables SNAT；串口原始TCP/UDP、默认单客户端独占；0仅不自动到期，仍绑定控制Session、失联/重启关闭；新入口来源限制、管理API网络隔离及成熟后端新数据链路加密需同步审核。
- 影响与风险：新增业务Service/API/WS、Probe能力/控制和辅助进程监管、WPF两页、构建打包；第三方新数据协议不能直接接入旧9001/RMT1；强制撤销/孤儿进程、旧内核/ABI、串口占用/流控与UDP回包语义均为PoC门槛。
