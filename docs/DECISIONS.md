...
## ADR-060 串口 TCP 首包注册鉴权与最小 GOST 适配

- 状态：**Accepted，2026-09-11**。用户明确外部主要为Windows电脑、通过网络调试工具直连，不额外安装依赖；选择注册包首包鉴权，并授权按讨论开始执行。该确认取代ADR-059中“外部客户端/握手方式待选”和本项仅调研的旧停止语句，不代表当前所有实现或设备准入已通过。
...
## ADR-059 内网穿透与串口透传调研提案

- 状态：**Proposed（2026-09-11，整体后端准入尚未定型）**。后续ADR-060已接受串口注册包方案并授权对应实施；下列A-only和客户端待确认语句仅保留历史授权记录，不得覆盖新授权。
