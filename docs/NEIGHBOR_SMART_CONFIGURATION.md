...
本轮实施基线：Accepted [ADR-059](DECISIONS.md)，局部取代ADR-056的手填普通配置和仅最新快照边界。工作分支已核对为 `GPT6API-TEST`；本轮无提交、推送、部署，未替换用户运行的Server/Probe。下面的测试协议对端、真实Linux Probe与厂商实机严格区分。
...
- 公开接口见[API](API.md) ADR-059节；新TASK、响应年龄及预设见[Protocol](PROTOCOL.md) ADR-059节。保持现有幂等/取消/配置ACK，不引入外部数据库或新数据面。
