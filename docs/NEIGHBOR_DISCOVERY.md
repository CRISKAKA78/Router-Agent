...
当前规范为ADR-056 + 2026-09-10 Accepted ADR-057。智能配置、自动直连扫描和近期记录取代本页原手工入口及仅快照边界，详见[智能邻居配置](NEIGHBOR_SMART_CONFIGURATION.md)。本页下方2026-09-10验证为历史证据，不作为本轮通过结论。

2026-09-10 原始ADR-056：用户已授权实施，并明确取消“上级/上联IP与MAC查询”；FNR100当前LAN1的接线不构成通用产品角色配置。
...
1. 在模板生成器启用邻居发现，选择在线参考设备与采集网络，默认生成local/lan域和30秒刷新周期；高级设置保留原始接口/域/端口/租约/只读命令。未验证时明确提示，不伪造自动成功。
2. 发布前协商Server能力；设备应用前检查Probe能力。FNR100预设须先显式只读测试，再确认所有应用同版本设备的作用范围；默认内核FDB可独立使用。
3. 默认查看Server“最近发现”，可切换最新快照。近期记录保留24小时/每设备1024条，不跨重启、Session或配置修订；刷新记录不主动扫描。
4. WPF默认显示所选域直连IPv4范围、主机数及预计耗时；点击即可扫描。高级自定义只允许直连子范围/24～/32，主机地址前缀会先规范化。旧Probe需更新后使用自动网络检测。
...
主动ARP使用原生以太网套接字，无arp-scan/nmap依赖；只绑定所选接口并校验其当前直连IPv4范围。每63ms最多发一次请求（不超过16请求/秒），发送结束再接收2秒，总期限30秒；一次仅一项扫描执行。只接受对应接口、已请求IP、目标IP/MAC及ARP头匹配的回复。扫描需要设备允许原始套接字；权限不足明确失败。ADR-057检测列出接口的多个IPv4前缀，用户明确选择一个扫描范围；Probe使用匹配该范围的源IPv4，并不自动遍历所有子网。
...
- Server只接收当前Session、已应用revision、匹配配置的域和LAN端口，保存最新快照并深拷贝返回；ADR-057另增加有界近期发现，重配置清空两者。没有新数据库或永久邻居资产库。
