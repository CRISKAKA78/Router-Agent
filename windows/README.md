...
在设置中保存服务器地址（新配置默认 `http://47.119.168.150:8888`，不附 `/api/v1`），后续启动自动连接。导航为设备、维护、文件、配置、设置；支持中文浅色/深色/系统主题。

维护显示 Web/SSH/Telnet 公共链接并打开系统或已配置的外部客户端。关闭或失效后不展示地址；SSH 账号/密码交给外部客户端，工作台不配置、保存或复制。文件管理默认打开 `/tmp/root`。没有内置终端、客户工具管理或通用任务入口；文件传输和配置结果留在各自页面。外部客户端与服务器维护不随工作台退出关闭。
...
| 工程 | 职责 |
| --- | --- |
| RouterWorkbench.Desktop | 原生 WPF 页面、布局和交互 |
