...
public sealed record NeighborRow(string Ip,string Mac,string Port,string Hostname,string Source,string State,string? Interface=null)
{
 ...
 public string StateText=>State switch {"cached"=>"缓存记录","reachable"=>"内核最近可达","lease"=>"仅租约","mac_only"=>"仅MAC记录","responded"=>"近期主动响应",_=>"未知"};
 public string SourceText=>string.Join(" / ",Source.Split('+').Select(s=>s switch {"arp"=>"ARP","ndp"=>"NDP","dhcp"=>"DHCP","fdb"=>"MAC转发表","active_arp"=>"主动ARP",_=>s}));
}
