...
internal static partial class Program
{
    ...
    private static async Task ManagedDeviceChecks(MainWindow window,ApiClient api,WorkspaceConnection connection,List<TestProbe> peers,int port)
    {
        ...
        await Eventually(()=>Task.FromResult(devices.Items.Cast<Device>().Any(d=>d.DeviceId=="managed-ui")),"admitted device reaches the dispatched native list");
        devices.SelectedItem=devices.Items.Cast<Device>().Single(d=>d.DeviceId=="managed-ui");Invoke(window,"Navigate","overview");
        ...
        Check(Field<TabControl>(window,"overviewTabs").Items.Cast<TabItem>().Select(t=>t.Header.ToString()).SequenceEqual(new[]{"系统信息","资源监控","基本信息","网络信息","接口状态","其他信息","LAN 下接设备","本机广播域设备","存储空间","连接历史"}),"built-in and custom groups are parallel; interface status contains port groups without sampling page");
        ...
        await peer.ReportAsync("switch",switchValues);
        var d=await api.GetAsync<Device>("devices/managed-ui");VisibilityChecks(d);Check(!props.Items.Cast<PropertyRow>().Any(p=>p.Key.StartsWith("switch_")),"physical port detail stays hidden in properties while specialized port table remains populated");Check(Field<TextBlock>((DeviceSummary)window.FindName("Summary"),"title").Text==d.DisplayName,"top summary displays administrator device identity");Check(d.DisplayName=="管理员机房路由器"&&d.Registration.Hostname=="探测名称","administrator name preserves raw registration");
