...
std::string RegisterPayload(const ClientConfig& config) {
    ...
    output << ",\"arch\":" << EscapeJsonString(config.arch)
           ...
           << ",\"capabilities\":[\"exec\",\"file\",\"tunnel\",\"router_config\",\"telemetry_v2\",\"managed_config_v1\",\"port_counters_v1\",\"neighbors_v1\",\"device_logs_v1\"]}";
    return output.str();
}
 ...
 if (frame->header.type==kTypeTunnelConnect || frame->header.type==kTypeTunnelClose) {
        ...
        if (frame->header.type == kTypeTask) {
            ...
            if (state == "conflict") {
                ...
                else if (task.type != "exec" && task.type != "router_config" && task.type != "device_logs" && task.type != "neighbor_scan" && task.type != "neighbor_cancel" && !file) reason = "unsupported task type";
