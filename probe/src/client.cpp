 ...
 if (frame->header.type==kTypeTunnelConnect || frame->header.type==kTypeTunnelClose) {
        ...
        if (frame->header.type == kTypeTask) {
            ...
            if (state == "conflict") {
                ...
                else if (task.type != "network_agent" && task.type != "exec" && task.type != "router_config" && task.type != "device_logs" && task.type != "neighbor_scan" && task.type != "neighbor_cancel" && task.type != "neighbor_inspect" && !file) reason = "unsupported task type";
