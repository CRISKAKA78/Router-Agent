...
func parseCellular(raw []byte) (device.Cellular, time.Duration, error) {
	...
	if len(raw) > 65536 || protocol.ValidObject(raw) != nil || json.Unmarshal(raw, &v) != nil || v.Event != "cellular" || v.Revision == 0 || v.Interval < 10 || v.Interval > 86400 || v.Age > 315360000000 || v.Ports == nil || len(v.Ports) > 16 || !slices.Contains([]string{"ok", "partial", "unavailable", "no_ports", "error"}, v.Status) || len(v.Reason) > 64 || strings.ContainsAny(v.Reason, "\x00\r\n") {
