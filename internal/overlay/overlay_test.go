...
func TestUncertainCannotReplayOrReconcilePendingTask(t *testing.T) {
	...
	n, _ = s.Get(n.ID)
	if n.Members[0].AppliedRevision != op.Revision {
		t.Fatalf("confirmed configuration not reflected in member: %+v", n.Members[0])
	}
	op, e = s.Start(n.ID, JoinRequest{DeviceID: "a"}, "stop")
	if e != nil || awaitOp(t, s, op.ID).State != "uncertain" {
		t.Fatal("expected uncertain stop", op, e)
	}
	if e = s.RemoveMember(n.ID, "a"); !errors.Is(e, ErrConflict) {
		t.Fatal("unconfirmed stop allowed removal", e)
	}
	if got, e = s.Reconcile(context.Background(), op.ID); e != nil || got.State != "reconciled" {
		t.Fatal("stop reconciliation", got, e)
	}
	if e = s.RemoveMember(n.ID, "a"); e != nil {
		t.Fatal("confirmed stopped member cannot be removed", e)
	}
	if up.stops != 1 || up.applies != 1 {
		t.Fatal("reconciliation replayed writes")
	}
}
