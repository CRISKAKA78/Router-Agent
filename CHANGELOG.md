...
## 2026-09-12 内网穿透与串口透传产品接入

- WPF新增内网穿透/串口透传两页，直连IPv4候选与手输、TCP/UDP内网代理、TCP串口注册鉴权、详情/复制/关闭；默认240分钟，0不限时仍受设备会话约束。
- 新增Forwarding API/WS主题、Probe可选forwarding_v1管理控制、GOST侧车监管及持久端口隔离；旧Maintenance保持。未鉴权数据不写UART，注册行不进串口。
- 已完成本地/隔离验证与交付包，未部署生产；UDP旧丢包与RSS门槛按用户要求暂停，真实UART/不同网关/长稳待验收。
