...
- 仓库上传成功后等待传输释放，不再误用下载本地提交标志阻塞引擎安装；保留完整性校验与不确定操作保护。
- 引擎异步启用后有界只读回查，不因首个过早快照立即误判，也不重放配置写入。
- 官方空路由回读丢失开关时仍保留待核对，不强行确认配置。

## 2026-09-12 精简固件文件目录兼容

- 修复FTV300等缺少stat命令的固件无法浏览目录：使用只读目录元数据回退，保留文件名和大小，无法获取的修改时间显示“未提供”。
- 读取失败时即使设备不返回错误文本，也显示任务状态、退出码与任务编号。目录Shell脚本统一LF，避免Windows源码行尾导致设备解析失败。

## 2026-09-12 Probe 双架构统一构建

- `probe-build.cmd` 一次生成 ARMv7 和 MIPS小端可执行文件，统一保存到编译机 `/root/router-agent/router-agent-armv7` 与 `router-agent-mipsel`；旧 gcc54 入口兼容转发。
- 修复中文项目路径下自动密码辅助脚本无法启动导致的“管道已结束”；改用临时原生 AskPass、SSH 登录预检和完整文件 SFTP 上传。
- 两架构独立记录、同一源码与默认接口；两份编译及检查成功后更新分名产物，编译失败保留此前成功文件，历史目录与旧产物不删除。

## 异地组网合入修正

- 保留AT、智能邻居、设备日志与既有维护入口，同时增加全局异地组网。
- 不确定启动回查确认后同步成员配置版本；停止经回查确认后允许移除成员，仍不重放旧操作。
- Probe受理network_agent任务时不再附带错误的“不支持任务”消息。

## 智能邻居、AT 与设备日志集成基线

- 同一版本的Server、Probe和WPF保留智能邻居、ATI/IMEI自动发现及实时/历史日志能力；保留现有服务器默认值、维护行为与两套独立Probe构建入口。
- GOST鉴权模块、补丁和PoC脚本同时纳入源码，但未接入产品API/Probe监管/WPF；后续实机测试独立进行，不宣称功能上线或准入通过。

## 2026-09-11 串口首包鉴权契约（未接入产品界面）

- 确认TCP串口采用文本注册包，Windows网络调试工具可直连；新增独立入口模块，错误注册/超时不连接后端，认证行不转发，支持独占和凭据轮换撤销。LAN仍保留TCP/UDP。
- 外部数据握手约定见ADR-063和PROTOCOL；当前只有隔离测试入口，正式Server API、Probe监管及WPF未接入，未部署。旧Maintenance接口与协议不变。
