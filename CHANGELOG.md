...
- 将异地组网改造、FM160蜂窝详情/趋势和GOST内网穿透/串口透传合并到同一Server/Probe/WPF源码，保留设备日志、智能邻居、文件管理和原维护入口；九个工作区并存。
- WebSocket默认/显式订阅同时支持networks与forwardings；Probe不因新增穿透侧车能力而丢失蜂窝、日志或组网能力。GOST产品接入范围及FM160 ADR编号按集成记录统一，既有服务器与双架构构建默认值不变。
- 本轮只交付源码与本机验证，不自动部署、不将已知UDP问题或实机待验项记作完成。
...
## 2026-09-12 异地组网成员配置与自动核实

- 新建网络支持自设/随机密码、按需查询、虚拟地址网段、初始节点默认 TCP/UDP 服务器11010、MTU1380；新成员启用 et0、私有模式、物理网卡绑定、系统转发、禁用对称打洞/UPnP及单线程。
- 添加成员支持多选，首次组网指定固定地址成员，其余 DHCP；成员独立设置 P2P、代理网段、手动路由与虚拟地址，运行中保存只重建该实例，停止成员只保存。
- 修复其他成员的未知操作阻塞已停止成员移除；后台自动核实未知结果、保留历史任务和停止替代关系，并为已成功配置的期望运行成员恢复丢失的临时引擎与稳定身份。
- 配置核实增加官方原始 TOML 证据回退，不再仅凭运行状态报成功，也不把空手动路由回读的 null 当 true。
- WPF 以左侧所选设备为观察视角，显示11列成员信息、中文 NAT、外部节点；独立链路页移除，拓扑改用路由器节点、缩放/拖动和实际计数驱动的流向动画。
- 新默认不覆盖旧网络配置；本轮源码与本机验证不代表生产部署或厂商双设备验收。
## 2026-09-12 FM160详细指标与锁定配置（独立工作树）

- 可选详情采集补充温度/供电、小区/邻区、PDP/APN/IP/DNS、PCC/SCC/MIMO/调制及已开启的射频测量；新增兼容能力门禁，旧模板默认不变。
- 蜂窝页按中文名称和分组展示详情；新增锁频段、锁频点、锁小区，未锁显示否，已锁显示配置值，查询失败不误报未锁。不提供锁定/解锁操作。
- 未定义的带宽编码、未启用的发射功率等如实标记，不臆造数值；保持固定信号曲线与快速刷新滚动位置。仅FM160-CN实测，尚未部署生产。

## 2026-09-12 FM160只读遥测与蜂窝趋势页（独立工作树）

- 同一模板可显式开启SIM/驻网/信号采集，按实际模块选择内置规则；首轮支持FM160-CN，未知型号保留身份查询。v2能力门禁保留旧模板与旧Probe默认行为。
- 蜂窝页自动发现端口后隐藏选择器；刷新保留滚动与选中，仅显示当前路径及查询字段，移除重复命令状态与说明。
- 当前值右侧新增RSRP、SINR、RSRQ固定范围曲线；按LTE/NR标记、量化近似值、未知断点和上下文隔离，不以0伪造缺测。当前为客户端有界历史，不是历史查询服务。
- 本条描述工作树内实现，不代表生产部署或其他模组适配完成。
