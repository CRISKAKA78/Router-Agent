...
public sealed class NeighborSettings
{
 ...
 public NeighborSettings Copy()=>new(){IntervalSeconds=IntervalSeconds,FdbCommand=FdbCommand,Domains=Domains.Select(d=>d.Copy()).ToList()};
}
public sealed class NeighborDomainSettings
{
 [JsonPropertyName("id")] public string Id {get;set;}="broadcast";
