...
/// <summary>File-format boundary for editable v1/v2 projects and the existing runtime template format.</summary>
public sealed class ProjectFiles
{
    ...
    public TemplateProject ReadProject(string json)
    {
        ...
        if (file.SchemaVersion is not (1 or 2) || file.Name is null || file.Attributes is null || file.Attributes.Count > 128)
        ...
        foreach (var row in file.Attributes)
        {
            if (row is null || row.Key is null || row.Name is null || row.Input is null || row.Sample is null ||
                ...
                row.Source is not ("command" or "nvram" or "uci" or "expression" or "rules") || (row.Source == "rules" && file.SchemaVersion == 1))
            ...
            {
                Key = row.Key, Name = row.Name, Input = row.Input, Sample = row.Sample, Timeout = row.Timeout.Value,
        ...
        return new TemplateProject { Name = file.Name, Attributes = attributes };
    }
    ...
    public TemplateProject FromTemplate(RuntimeTemplate template)
    {
        ...
        foreach (var (key, property) in template.Properties)
        {
            ...
            {
                Key = key, Name = property.Name, Source = Enum.Parse<AttributeSource>(source, true), Input = input,
    ...
    private static TemplateProject ReadTemplate(ImportedFile file)
    {
        ...
        var project = new TemplateProject { Name = file.Name };
