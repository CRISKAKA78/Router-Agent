...
public sealed class ProjectFiles
{
    ...
    public TemplateProject ReadProject(string json)
    {
        ...
        return new TemplateProject { Name = file.Name, Attributes = attributes, Presentation=file.Presentation,NeighborProbe=file.NeighborProbe,CellularProbe=file.CellularProbe,SwitchProbe=file.SwitchProbe,Monitoring=file.Monitoring };
    }

    public TemplateProject FromTemplate(RuntimeTemplate template)
    {
        var project = TemplateCompiler.EmptyProject();
        project.CellularProbe=template.CellularProbe?.Copy();project.NeighborProbe=template.NeighborProbe?.Copy();project.Presentation=template.Presentation?.Copy();project.SwitchProbe=template.SwitchProbe?.Copy();
 project.Name = template.Name; project.Monitoring=template.Monitoring?.Copy();
    ...
    private static TemplateProject ReadTemplate(ImportedFile file)
    {
        ...
        var project = new TemplateProject { Name = file.Name,Presentation=file.Presentation,NeighborProbe=file.NeighborProbe,CellularProbe=file.CellularProbe,SwitchProbe=file.SwitchProbe,Monitoring=file.Monitoring };
 ...
 [JsonPropertyName("cellular_probe")] public CellularSettings? CellularProbe{get;set;}
