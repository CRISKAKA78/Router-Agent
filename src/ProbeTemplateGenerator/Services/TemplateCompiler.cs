...
public sealed partial class TemplateCompiler
{
 ...
 ValidateNeighbors(project,issues);
        ...
        if ((display.Count==0 && project.Monitoring is null && project.Presentation is null && project.SwitchProbe is null && project.NeighborProbe is null && project.CellularProbe is null) || display.Count > 38 || display.Count(row => !Standard.ContainsKey(row.Key)) > 32)
    ...
    public RuntimeTemplate Compile(TemplateProject project)
    {
        ...
        var output = new RuntimeTemplate { CellularProbe=project.CellularProbe?.Copy(), NeighborProbe=project.NeighborProbe?.Copy(),Presentation=project.Presentation?.Copy(),SwitchProbe=project.SwitchProbe?.Copy(),Name = project.Name, Monitoring = project.Monitoring?.Copy() };
